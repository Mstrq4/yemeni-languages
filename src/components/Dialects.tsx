...
export default function Dialects() {
  return (
    <section id="dialects" className="py-16 sm:py-20 md:py-28 bg-stone-50 dark:bg-[#030712]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        >
          ...
          <p className="text-sm sm:text-base md:text-lg text-stone-600 dark:text-stone-300 max-w-2xl mx-auto">
            أربع لهجات تفرّعت من اللغة اليمنية القديمة، لكلٍّ منها مملكتها ومنطقة نفوذها
          </p>
        </motion.div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
          {dialects.map((d: any, i: number) => (
            <motion.div
              key={d.name}
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h3 className="text-lg sm:text-xl font-bold text-amber-600 dark:text-amber-500">{d.name}</h3>
                  <p className="text-xs sm:text-sm text-stone-500 dark:text-stone-400 mt-1">{d.region}</p>
                </div>
              ...
              <p className="text-xs sm:text-sm leading-relaxed text-stone-600 dark:text-stone-300">{d.description}</p>
            </motion.div>
          ))}
        </div>
      </div>
